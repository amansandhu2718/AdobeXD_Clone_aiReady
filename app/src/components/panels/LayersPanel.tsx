...
import { ChevronDown, ChevronRight, Copy, Eye, EyeOff, ImageIcon, Lock, LockOpen, Trash2 } from 'lucide-react'
...
function LayerElementRow({
  ...
  onDuplicate: (frameId: string, elementId: string) => void
    ...
    <>
      ...
      >
        ...
        <button
          className="icon-button"
          onClick={() =>
            selectedElementId === element.id ? onDelete(frameId, element.id) : onDuplicate(frameId, element.id)
          }
          title={selectedElementId === element.id ? 'Delete' : 'Duplicate'}
          type="button"
        >
          {selectedElementId === element.id ? <Trash2 size={13} /> : <Copy size={13} />}
        </button>
      </div>
      {!collapsed &&
        (
